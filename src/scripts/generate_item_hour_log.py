...
def compute_play_rate_and_new_pctr(item_hour_log):
    ...
    np.random.seed(42)
    item_hour_log["photo_time"] = np.random.uniform(30, 300, size=len(
        item_hour_log))  # Simulate content duration (30-300 seconds)
    item_hour_log["play_time"] = item_hour_log["clicks"] * np.random.uniform(10, 30, size=len(
        item_hour_log))  # Simulate play_time
...
if __name__ == "__main__":
    ...
    for dataset_folder in dataset_folders:
        ...
        generate_item_hour_log(base_path, dataset_folder)
